...
public class Vaporize implements ElementReaction {
    @Override
    public boolean appliesTo(LivingEntity target, DamageSource source) {
        return  ("fire_magic".equals(source.getMsgId()) && target.getData(SpellAttachments.WATER_ELEMENT).getValue() > 0)
                || ("water_magic".equals(source.getMsgId()) && target.getData(SpellAttachments.FIRE_ELEMENT).getValue() > 0);
    }
