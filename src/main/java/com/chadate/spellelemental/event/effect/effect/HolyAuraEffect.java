...
public class HolyAuraEffect {
    private static final int PARTICLE_COUNT = 2; // 每tick只生成2个粒子
    private static final Random random = new Random();
    private static final DustParticleOptions HOLY_PARTICLE = new DustParticleOptions(
        new Vector3f(1.0f, 0.9f, 0.2f), // 金色
        1.0f
    );

    public static void applyHolyAura(EntityTickEvent.Pre event) {
        if (!(event.getEntity() instanceof LivingEntity entity)) return;
        
        // 检查实体是否有神圣元素附着
        if (entity.getData(SpellAttachments.HOLY_ELEMENT).getValue() <= 0) return;
