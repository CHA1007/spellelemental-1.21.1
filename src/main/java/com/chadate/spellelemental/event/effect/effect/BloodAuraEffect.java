...
public class BloodAuraEffect {
    private static final int PARTICLE_COUNT = 2; // 每tick只生成2个粒子
    private static final Random random = new Random();
    private static final DustParticleOptions BLOOD_PARTICLE = new DustParticleOptions(
        new Vector3f(0.8f, 0.0f, 0.0f), // 深红色
        1.0f
    );

    public static void applyBloodAura(EntityTickEvent.Pre event) {
        if (!(event.getEntity() instanceof LivingEntity entity)) return;
        
        // 检查实体是否有血元素附着
        if (entity.getData(SpellAttachments.BLOOD_ELEMENT).getValue() <= 0) return;
