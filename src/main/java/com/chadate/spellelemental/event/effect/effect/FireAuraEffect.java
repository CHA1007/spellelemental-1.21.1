...
public class FireAuraEffect {
    private static final int PARTICLE_COUNT = 2; // 每tick只生成2个粒子
    private static final Random random = new Random();

    public static void applyFireAura(EntityTickEvent.Pre event) {
        if (!(event.getEntity() instanceof LivingEntity entity)) return;
        
        // 检查实体是否有火元素附着
        if (entity.getData(SpellAttachments.FIRE_ELEMENT).getValue() <= 0) return;
