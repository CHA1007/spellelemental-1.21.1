...
public class WaterElementRenderer extends BaseElementRenderer {
    public WaterElementRenderer(float scale, float alpha) {
        super("water", scale, alpha);
    }

    @Override
    public boolean shouldRender(LivingEntity entity) {
        return entity.getData(SpellAttachments.WATER_ELEMENT).getValue() > 0;
    }
